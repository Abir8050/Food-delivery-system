...
// Checking if created_at exists, if not, fallback to id
$res_check = mysqli_query($conn, "SHOW COLUMNS FROM promotional_offers LIKE 'created_at'");
if (mysqli_num_rows($res_check) == 0) {
  $sql = "SELECT * FROM promotional_offers WHERE restaurant_owner_id = '$restaurantOwnerId' ORDER BY id DESC";
}
  ...
  <div class="card">
    ...
    <form action="process_discount.php" method="post" novalidate>
      <div class="form-group">
        ...
        <input type="text" id="name" name="name" placeholder="e.g. Weekend Special">
      ...
      <div class="form-group">
        ...
        <textarea id="description" name="description" rows="3" placeholder="What's included in this offer?"></textarea>
      ...
      <div class="form-group">
        ...
        <input type="number" id="discount_percent" name="discount_percent" min="1" max="100" placeholder="e.g. 15">
      ...
      <div class="form-group" style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.5rem;">
        ...
        <?php if (isset($_SESSION['errors']['active']))
          echo "<span class='error-msg'>{$_SESSION['errors']['active']}</span>"; ?>
      </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($result)): ?>
              <tr>
                ...
                <td style="text-align: center;">
                  <span
                    style="background: #fdf2f2; color: #cf222e; padding: 0.2rem 0.6rem; border-radius: 4px; font-weight: 800;">
                    <?= htmlspecialchars($row['discount_percent']) ?>%
                  </span>
                </td>
                <td style="font-size: 0.85rem;">
                  <div style="display: flex; flex-direction: column;">
                    <span><i class="far fa-calendar-alt" style="width: 15px;"></i>
                      <?= date('M d', strtotime($row['start_date'])) ?></span>
                    <span><i class="far fa-calendar-check" style="width: 15px;"></i>
                      <?= date('M d', strtotime($row['end_date'])) ?></span>
                  </div>
                </td>
                <td style="text-align: center;">
                  <?php if ($row['active']): ?>
                    <span style="color: #1a7f37;"><i class="fas fa-check-circle"></i> Active</span>
                  <?php else: ?>
